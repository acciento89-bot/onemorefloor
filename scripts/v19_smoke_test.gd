...
func _run() -> void:
	...
	var compatible_version := (
		...
		or project_text.contains("config/version=\"1.8.1-raster-reference\"")
	)
	...
	var ios_ok := (
		...
		or (export_text.contains("application/short_version=\"1.8.1\"") and export_text.contains("application/version=\"14\""))
	)
	if not ios_ok:
	...
	var android_ok := (
		...
		or (export_text.contains("version/name=\"1.8.1\"") and export_text.contains("version/code=14"))
	)
	if not android_ok:
