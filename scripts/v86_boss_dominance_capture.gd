...
const CAPTURE_DIR := "res://artifacts/v163_boss_dominance"
...
func _run() -> void:
	...
	if not bool(world.call("production_boss_dominance_ready")):
		_fail("v1.63 r2.1 boss dominance world not ready")
		return
	...
	world.call("_animate_boss_dominance")
	if not await _save_frame("r21_boss_intro"):
		return
	...
	world.call("_animate_boss_frame")
	world.call("_animate_boss_dominance")
	if not await _save_frame("r21_boss_fan"):
		return
	...
	world.call("_animate_boss_frame")
	world.call("_animate_boss_dominance")
	if not await _save_frame("r21_boss_crown_slam"):
		return

	print("v1.63 boss dominance r2.1 visual capture passed")
	world.queue_free()
	await process_frame
	quit(0)
...
func _save_frame(stem: String) -> bool:
	...
	if image == null or image.is_empty():
		_fail("empty boss r2.1 image: %s" % stem)
		return false
	var output := "%s/%s.png" % [CAPTURE_DIR, stem]
	if image.save_png(ProjectSettings.globalize_path(output)) != OK:
		_fail("could not save boss r2.1 image: %s" % stem)
		return false
	print("V163_BOSS_R21_CAPTURE:%s:%s:%dx%d" % [stem, output, image.get_width(), image.get_height()])
	return true

func _fail(message: String) -> void:
	push_error("V163_BOSS_R21_CAPTURE_FAIL:%s" % message)
	quit(1)
