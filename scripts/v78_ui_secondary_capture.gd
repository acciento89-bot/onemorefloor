...
const CAPTURE_SCREENS := ["vault", "missions", "pass", "store"]
const CAPTURE_DIR := "res://artifacts/v162_secondary"
...
func _run() -> void:
	...
	for screen in CAPTURE_SCREENS:
		...
		if result != OK:
			...
			return
		print("V78_UI_CAPTURE:%s:%s:%dx%d" % [screen, output, image.get_width(), image.get_height()])

	print("v1.62 secondary menu visual capture passed")
	game.queue_free()
	await process_frame
	quit(0)

func _fail(message: String) -> void:
	push_error("V78_UI_CAPTURE_FAIL:%s" % message)
	quit(1)
