...
func _run() -> void:
	...
	if not bool(world.call("production_environment_depth_ready")):
		_fail("v1.65 environment depth r1.1 world is not ready")
		return
	var snapshot: Dictionary = world.call("debug_snapshot")
	if String(snapshot.get("production_environment_depth_version", "")) != "1.65-environment-depth-r1.1":
		_fail("active v1.65 review is not using r1.1")
		return
	...
	# Keep the historical workflow grep token while explicitly recording r1.1.
	print("v1.65 environment surface and depth r1 smoke test passed (r1.1 active)")
	world.queue_free()
	await process_frame
	quit(0)
