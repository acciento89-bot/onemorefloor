...
func _run() -> void:
	...
	for floor_no in FLOORS:
		...
		if image == null or image.is_empty():
			_fail("empty capture on floor %s" % floor_no)
			return
		var output := "%s/floor%02d.png" % [CAPTURE_DIR, floor_no]
		if image.save_png(ProjectSettings.globalize_path(output)) != OK:
			_fail("could not save floor %s" % floor_no)
			return
		print("V74_TOWER_CAPTURE:%02d:%s:%dx%d" % [floor_no, output, image.get_width(), image.get_height()])

	print("v1.60 authored tower realm visual capture passed")
	world.queue_free()
	await process_frame
	quit(0)

func _fail(message: String) -> void:
	push_error("V74_TOWER_CAPTURE_FAIL:%s" % message)
	quit(1)
