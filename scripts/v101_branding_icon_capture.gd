...
# ONE MORE FLOOR v1.74 r1.4 — Product Identity candidate.
...
# materials and deliberate forward-facing Hero presentation.
# r1: rejected — chamber dominated the icon.
# r1.1: rejected — isolated gameplay pose still read too small/side-on.
# r1.2: rejected — correct Hero source, crop too tight and lost Hood/head.
# r1.3: rejected — full-body composition was correct but too small at 60 px.
# r1.4: deliberate Hero bust: Hood/eyes/shoulders/chest/blade own the mark.
...
func _run() -> void:
	...
	stage.actor_model.scale = Vector3.ONE * 1.90
	stage.gameplay_actor_factory.animate_player(stage.actor_model, 7.5, 0.0, 0.0, 0.0)

	# Bust portrait: intentionally crop lower body. The identity-bearing upper half
	# fills the square so the Hood and eyes survive 60 px home-screen rendering.
	stage.camera.projection = Camera3D.PROJECTION_PERSPECTIVE
	stage.camera.fov = 32.0
	stage.camera.position = Vector3(0.0, 4.20, 5.55)
	stage.camera.look_at(Vector3(0.0, 4.14, 0.02), Vector3.UP)
	...
	print("V174_BRANDING_ICON:accepted-hero-bust-r1.4:1024x1024:opaque")
...
func _build_branding_backdrop() -> void:
	...
	back.position = Vector3(0.0, 4.18, -1.15)
	...
	halo_mesh.radius = 1.02
	halo_mesh.height = 2.04
	...
	halo.position = Vector3(0.0, 4.12, -0.96)
	halo.scale = Vector3(1.22, 1.22, 0.08)
...
func _build_branding_lights() -> void:
	...
	key.position = Vector3(-1.25, 4.85, 1.85)
	...
	rim.position = Vector3(1.65, 4.35, 0.65)
	rim.light_color = Color("9c78dc")
	rim.light_energy = 1.70
	rim.omni_range = 3.5
	...
	warm.position = Vector3(1.20, 4.00, 2.10)
	warm.light_color = Color("e1ad66")
	warm.light_energy = 0.90
	warm.omni_range = 3.2
