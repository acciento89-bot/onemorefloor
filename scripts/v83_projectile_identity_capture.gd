...
const WorldV163 = preload("res://scripts/world3d_chamber_v163_projectile_identity.gd")
const CAPTURE_DIR := "res://artifacts/v163_projectile_identity"
...
func _run() -> void:
	...
	var world = WorldV163.new()
	root.add_child(world)
	world.set_active(true)
	for _i in range(10):
		await process_frame
	if not bool(world.call("production_projectile_identity_ready")):
		_fail("v1.63 projectile identity world not ready")
		return
	...
	if not await _save_frame("r1_player_projectiles"):
		return
	...
	_clear_transients(world)
	if not await _save_frame("r1_enemy_projectiles"):
		return
	...
	_clear_transients(world)
	if not await _save_frame("r1_mixed_projectiles"):
		return

	print("v1.63 projectile identity r1 visual capture passed")
	world.queue_free()
	await process_frame
	quit(0)
...
func _save_frame(stem: String) -> bool:
	...
	if image == null or image.is_empty():
		_fail("empty r1 projectile capture for %s" % stem)
		return false
	var output := "%s/%s.png" % [CAPTURE_DIR, stem]
	if image.save_png(ProjectSettings.globalize_path(output)) != OK:
		_fail("could not save r1 projectile capture %s" % stem)
		return false
	print("V163_PROJECTILE_R1_CAPTURE:%s:%s:%dx%d" % [stem, output, image.get_width(), image.get_height()])
	return true

func _fail(message: String) -> void:
	push_error("V163_PROJECTILE_R1_CAPTURE_FAIL:%s" % message)
	quit(1)
