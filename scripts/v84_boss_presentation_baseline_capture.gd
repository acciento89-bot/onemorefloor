...
const CAPTURE_DIR := "res://artifacts/v163_boss_baseline"
...
func _run() -> void:
	...
	if not bool(world.call("production_projectile_identity_ready")):
		_fail("accepted r1 world not ready before boss baseline")
		return
	...
	# Intro: isolate the inherited persistent boss frame from realm transition VFX.
	...
	if not await _save_frame("baseline_boss_intro"):
		return

	# Fan/focus: preserve the accepted r3.2 directional tell and r1 hostile shards,
	# exposing how the old halo/beam/crown frame competes with actual combat intent.
	...
	world.call("_animate_boss_frame")
	if not await _save_frame("baseline_boss_fan"):
		return

	# Crown/slam: same persistent frame, but with the accepted radial-teeth danger
	# tell. This is the key check that decorative boss identity stays separate from
	# gameplay-significant slam language.
	...
	world.boss_intro_timer = 0.0
	world.call("_animate_boss_frame")
	if not await _save_frame("baseline_boss_crown_slam"):
		return

	print("v1.63 boss presentation baseline visual capture passed")
	world.queue_free()
	await process_frame
	quit(0)
...
func _save_frame(stem: String) -> bool:
	...
	if image == null or image.is_empty():
		_fail("empty boss baseline image: %s" % stem)
		return false
	var output := "%s/%s.png" % [CAPTURE_DIR, stem]
	if image.save_png(ProjectSettings.globalize_path(output)) != OK:
		_fail("could not save boss baseline image: %s" % stem)
		return false
	print("V163_BOSS_BASELINE_CAPTURE:%s:%s:%dx%d" % [stem, output, image.get_width(), image.get_height()])
	return true

func _fail(message: String) -> void:
	push_error("V163_BOSS_BASELINE_CAPTURE_FAIL:%s" % message)
	quit(1)
