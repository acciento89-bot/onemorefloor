...
func _build_home_stage() -> void:
	...
	_make_box(stage_root, "HomeGateRecess", Vector3(4.7, 4.3, 0.20), mat_stone_mid, Vector3(0.0, 2.38, -3.12))
	...
	_make_box(stage_root, "HomeLintel", Vector3(3.35, 0.18, 0.28), mat_brass, Vector3(0.0, 4.30, -2.87))
	for x in [-2.35, 2.35]:
...
func _build_forge_stage() -> void:
	...
	_make_box(stage_root, "ForgeHearth", Vector3(1.62, 1.72, 0.30), mat_stone_hi, Vector3(-1.30, 2.02, -2.82))
	_make_box(stage_root, "ForgeMouth", Vector3(0.98, 0.84, 0.07), mat_ember, Vector3(-1.30, 1.80, -2.56))
	_make_box(stage_root, "ForgeHood", Vector3(1.80, 0.40, 0.30), mat_stone, Vector3(-1.30, 3.18, -2.80))
	_add_torch(Vector3(-1.30, 2.00, -2.40), Color("ff7433"), 0.60)

	# Open weapon rack: rails instead of the previous giant black rectangle.
	var rack := Node3D.new()
	rack.name = "ForgeRack"
	rack.position = Vector3(1.70, 2.20, -2.90)
	stage_root.add_child(rack)
	_make_box(rack, "RackLeft", Vector3(0.10, 2.22, 0.10), mat_dark_metal, Vector3(-0.78, 0.0, 0.0))
	_make_box(rack, "RackRight", Vector3(0.10, 2.22, 0.10), mat_dark_metal, Vector3(0.78, 0.0, 0.0))
	_make_box(rack, "RackTop", Vector3(1.66, 0.10, 0.10), mat_brass, Vector3(0.0, 1.05, 0.0))
	_make_box(rack, "RackBottom", Vector3(1.66, 0.10, 0.10), mat_dark_metal, Vector3(0.0, -1.05, 0.0))
	for i in range(4):
		var x := -0.50 + float(i) * 0.33
		_make_box(rack, "ForgeWeapon%02d" % i, Vector3(0.055, 1.34, 0.055), mat_brass if i % 2 == 0 else mat_stone_hi, Vector3(x, 0.02, 0.18), Vector3(0.0, 0.0, -0.16 + float(i) * 0.10))
	_make_box(stage_root, "ForgeAnvilTop", Vector3(1.02, 0.18, 0.38), mat_stone_hi, Vector3(0.10, 1.28, -2.54))
	_make_box(stage_root, "ForgeAnvilStem", Vector3(0.38, 0.72, 0.30), mat_dark_metal, Vector3(0.10, 0.86, -2.62))
	_add_torch(Vector3(2.55, 3.05, -2.52), Color("e8a05a"), 0.36)
