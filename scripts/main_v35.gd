...
func cash_out() -> void:
	_record_v35_run_summary("cash_out")
	super.cash_out()
	if release_guard != null:
		release_guard.backup_last_good()

func die() -> void:
	_record_v35_run_summary("death")
	super.die()
	if release_guard != null:
		release_guard.backup_last_good()
...
func _draw_room_floor() -> void:
	...
	for row in range(14):
		...
		draw_line(Vector2(ARENA.position.x, y), Vector2(ARENA.end.x, y), Color(accent, 0.045 + float(row % 3) * 0.012), 1.0)
	for col in range(10):
	...
	match area:
		...
		"CELESTIAL GRAVE":
			for i in range(24):
				...
				var rr := 85.0 + float((i * 47) % 180)
				draw_circle(center + Vector2.from_angle(a)*rr, 1.5 + float(i%3), Color(accent,0.22))
