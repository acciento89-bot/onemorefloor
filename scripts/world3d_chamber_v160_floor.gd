...
func _hide_legacy_prototype_grid() -> int:
	...
	for child in get_children():
		...
		var size := box.size
		var is_long_floor_strip := size.y <= 0.055 and (size.x >= 7.0 or size.z >= 10.0)
...
func _build_lower_floor(root_node: Node3D) -> void:
	...
	for row in range(5):
		var z := -5.15 + float(row) * 2.58
		var offset := 0.16 if row % 2 == 0 else -0.12
		for col in range(3):
			var x := -3.02 + float(col) * 3.02 + offset
			var mat := lower_floor_a if (row + col) % 2 == 0 else lower_floor_b
...
func _build_ossuary_floor(root_node: Node3D) -> void:
	...
	for row in range(4):
		var z := -4.85 + float(row) * 3.12
		for side in [-1.0, 1.0]:
			var x := side * (2.18 + (0.10 if row % 2 == 0 else -0.08))
			var mat := ossuary_floor_a if (row + (0 if side < 0.0 else 1)) % 2 == 0 else ossuary_floor_b
...
func _build_iron_floor(root_node: Node3D) -> void:
	...
	for row in range(4):
		var z := -4.75 + float(row) * 3.10
		var offset := 0.18 if row % 2 == 1 else -0.12
		for col in range(3):
			var x := -3.0 + float(col) * 3.0 + offset
			var mat := iron_floor_b if (row == 1 and col == 2) or (row == 3 and col == 0) else iron_floor_a
...
func _build_rift_floor(root_node: Node3D) -> void:
	...
	for index in range(7):
		var z := -5.15 + float(index) * 1.70
		var x := sin(float(index) * 1.55) * 0.28
		var width := 5.75 - float(index % 3) * 0.42
		var mat := rift_floor_a if index % 2 == 0 else rift_floor_b
...
func _build_spire_floor(root_node: Node3D) -> void:
	...
	for index in range(6):
		var z := -4.85 + float(index) * 2.05
		var x := 0.10 if index % 2 == 0 else -0.08
		var mat := spire_floor_a if index % 2 == 0 else spire_floor_b
