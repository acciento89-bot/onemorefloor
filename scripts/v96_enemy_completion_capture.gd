...
# v1.69 matched visual evidence: accepted v1.68 r1.1 combat world versus the
# authored enemy/boss completion candidate. Fixed offscreen 720x1280 viewport.
...
const CAPTURE_DIR := "res://artifacts/v169_enemy_completion"
...
func _run() -> void:
	...
	capture_viewport.name = "V169FixedCaptureViewport"
	...
	print("v1.69 enemy and boss matched visual capture passed")
...
func _capture_world(world_script: Script, prefix: String) -> bool:
	...
	for _i in range(10):
		await process_frame
	if prefix == "after" and not bool(world.call("production_enemy_completion_ready")):
		_fail("candidate v1.69 world is not ready")
		return false
	if prefix == "before" and not bool(world.call("production_character_completion_ready")):
		_fail("accepted v1.68 world is not ready")
		return false
...
func _save_frame(stem: String) -> bool:
	...
	if image == null or image.is_empty():
		_fail("empty v1.69 image: %s" % stem)
		return false
	if image.get_width() != CAPTURE_SIZE.x or image.get_height() != CAPTURE_SIZE.y:
		_fail("invalid v1.69 capture size for %s: %dx%d" % [stem, image.get_width(), image.get_height()])
		return false
	var output := "%s/%s.png" % [CAPTURE_DIR, stem]
	if image.save_png(ProjectSettings.globalize_path(output)) != OK:
		_fail("could not save v1.69 image: %s" % stem)
		return false
	print("V169_ENEMY_COMPLETION:%s:%s:%dx%d" % [stem, output, image.get_width(), image.get_height()])
	return true

func _fail(message: String) -> void:
	push_error("V169_ENEMY_COMPLETION_CAPTURE_FAIL:%s" % message)
	quit(1)
